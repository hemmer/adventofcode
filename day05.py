...
def isNice(string):
    ...
    return numVowels >= 3 and doubleChar
...
print sum(isNice(string) for string in input_str.split('\n'))
...
def isNice2(string):
    ...
    for i in xrange(len(string)-2):
        pair = string[i:i+2]
        for j in xrange(i+2, len(string)):
            if string[j:j+2] == pair:
                twoPair = True

    return twoPair and doubleChar
...
print sum(isNice2(string) for string in input_str.split('\n'))
